...
const productController = {
  ...
  searchProductsCreatedByUser: async (req, res) => {
    const userId = req.params.userId;
    const searchQuery = req.query.name; // Tên sản phẩm cần tìm kiếm
  
    try {
      const products = await ProductModel.find({ createdBy: userId, name: { $regex: searchQuery, $options: 'i' } }).populate('category');
      res.status(200).json({ data: products });
    } catch (err) {
      res.status(500).json(err);
    }
