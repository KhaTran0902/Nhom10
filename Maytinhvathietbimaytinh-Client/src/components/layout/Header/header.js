...
import { BarsOutlined, BellOutlined, NotificationTwoTone, ShoppingOutlined } from '@ant-design/icons';
import { Avatar, Badge, Col, Drawer, Layout, List, Modal, Popover, Row, Select } from 'antd';
...
function Topbar() {
  ...
  const handleSearch = async (value) => {
    try {
        ...
        const filteredProducts = data.docs.filter(product => product.status === true);
  ...
  return (
    ...
    >
      <div className="">
        <img style={{ color: "#000000", fontSize: 15, height: 55, cursor: "pointer" }} src="https://upload.wikimedia.org/wikipedia/commons/2/2d/Sky_Store_Logo_2020.png" onClick={() => handleLink("/home")}></img>
      </div>
      ...
      <div className={styles.navmenu} style={{ marginLeft: 15 }}>
        ...
        <NavLink className={styles.navlink} to="/product-list/653a6c4db32da1285f701662" activeStyle>
          Sản phẩm
        </NavLink>
        <NavLink className={styles.navlink} to="/comparing" activeStyle>
          So Sánh Giá
        </NavLink>
